...
*         (\hbar = m_e = c = 1)                                        *
*     The first subroutine converts the conductivities to CGS units.   *
...
C------------------------ WARNING --------------------------------------
c WE HAVE MOVED THE CALL OF CONDI (IN ORDER TO INCLUDE THE THERMAL 
c CONDUCTIVITY OF NON-SF PHONONS/IONS) TO POTEKHIN2019.F SUBROUTINE.     
C MOREOVER, THE CONVERSION TO ORDINARY PHYSICAL (CGSE) UNITS OF RKAPPA, 
C RTKAPPA AND RHKAPPA IS ALSO MOVED TO potekhin2019.f subroutine.      
c ----------------------------------------------------------------------
...
*   ---------------------------------------------------------------    *
! Previously CONDCONV
      subroutine cond_outer_crust_electrons(T6,RHO,B,Zion,CMI,Zimp,
     *  SIGMA,RKAPPA,QJ,SIGMAT,RTKAPPA,QJT,SIGMAH,RHKAPPA,QJH)
...
*  coefficients SIGMA,KAPPA,Q (in Relativistic units):                 *
      call cond_electrons(TEMR,DENRI,B0,Zion,CMI,Zimp,
...
* Stems from v.17.02.08. Diff.: more correct account of THtoEL in EECOR
      subroutine cond_electrons(TEMR,DENRI,B0,Zion,CMI,Zimp,
     & RSIGMA,RLET,RLTT,RQ,RKAPPA,
     & RTSIGMA,RTLET,RTLTT,RTQ,RTKAPPA,
     & RHSIGMA,RHLET,RHLTT,RHQ,RHKAPPA)
...
*   Output: Fn=\int [(E-\mu)/T]^n [N_e(E)/E] \tau_{eff}(E) df_0(E)
      ...
      data TAIL1/10.d0/,TAIL2/10.d0/,NM/32/
*        Nodes and weights of the Gauss formula:
      data AXI/.32425342340381d0, .61337143270059d0,
     *         .83603 11073 2664d0, .96816 02395 0763d0/,
     *  AWI/.31234707704d0, .26061069640294d0,
     *      .18064816069486d0, .08127438836157d0/,NXI/4/
...
c      if (PCL**2.gt.4.d2*B) then ! Non-magnetic case
         CLlong=CLeff
         CLtran=CLeff
         SN=1.d0
         goto 50
...
* Correction for partial degeneracy:
      ...
      subroutine cond_outer_crust_ions(TEMR,DENRI,Zion,CMI,RKAPi)
...
*   ===================  AUXILIARY SUBROUTINES  ====================   *
      ...
      FUNCTION expint(x,n) ! = e^x E_{N+1}(x)
      implicit none
      INTEGER n,MAXIT
      REAL*8 expint,x,EPS,FPMIN,EULER
      PARAMETER (MAXIT=100,EPS=1.e-4,FPMIN=1.e-30,EULER=.5772156649)
      INTEGER i,ii,nm1,np
      ...
      if(np.lt.0.or.x.lt.0..or.(x.eq.0..and.(np.eq.0.or.np.eq.1)))then
...
*   ===================  AUXILIARY SUBROUTINES  ====================   *
c      function EXPINT(XI,L) ! = e^XI E_{L+1}(XI)
c      implicit double precision (A-H), double precision (O-Z)
c      save
c      data GAMMA/.5772156649D0/,Nrep/21/
c      if (XI.ge.1.) then ! continued fraction
c         CL=L
c         CI=Nrep
c         C=0.
c        do I=Nrep,1,-1
...
c         do 22 M=0,Nrep
c         CM=CM+1.d0
c        if (M.ne.0) CMX=-CMX*XI/CM ! (-XI)^M/M!
c        if (M.ne.L) then ! Actually DQ=-deltaQ0
c           DQ=CMX/(CM-CL)
c        else
c           DQ=CMX*(dlog(XI+1.d-20)-PSI)
c        endif
c         Q0=Q0-DQ
c   22    continue
c         Q0=exp(XI)*Q0
