program main
  ...
  use output, only: output_checkpoint
  use OMP_LIB

  implicit none

  real*8 :: dt, dtb
  real*8 :: time, time_b, time_read
  real*8 :: tau_output_serv, tau_output3D_serv
  integer :: nradial, nangular
  integer :: ib, it, ic, nout, nout3D, noutT, nout3DT, it_read
  logical :: checkpoint_condition1, checkpoint_condition2
  ...
  call initial_condition(it_read, time_read)
  time = time_read
 ...
 ! Thermal evolution
  do while (time < final_time)
    ...
    if (itevol .eqv. .true.) then
      ...
      ! Write the checkpoint given by the indicated frequency in the output, if any
      checkpoint_condition1 = (checkpnt_freq .gt. 0) .and. (mod(it,checkpnt_freq) .eq. 0)
      ! Avoid writing at t=0 or at the beginning of a resumed run
      checkpoint_condition2 = (it .gt. resume_checkpnt_number) .and. (time .ne. 0d0)

      if (checkpoint_condition1 .and. checkpoint_condition2) then
        ! time in simulation unit
        call output_checkpoint(it, time) 
      endif 
