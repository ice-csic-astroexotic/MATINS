...
* EOSPMAG=pressure [Mbar] of magnetized electron-ion Coulomb plasma
* PRESSMAG=pressure [rel.units] of magnetized electron ideal gas
...
C%C      end
      ...
      subroutine EOSMAG(Zion,CMI,RHO,TEMP,GAMAG,cv_ion,cv_e)
cc     *  DENS,GAMI,CHI,TPT,LIQSOL,PnkT,UNkT,SNk,CV,CHIR,CHIT)
*                                                       Version 06.07.12
...
*        if (FC1=min in solid) then solid regime, output LIQSOL=5
     ...
     *   RSIMELT=140.) ! ion density parameter of quantum melting
      ...
      B=GAMAG/BOHR**2 ! magnetic field in rel.un.
...
* is as crude as the linear mixing model for Fharm employed below.
      if ((LIQSOL.eq.0.or.LIQSOL.eq.2).and.TPT2.gt.18.) then
        LIQSOL=1
        TPT2=18.d0
c        print*,rho,temp,gamag
c        stop'EOSMAG: too strong quantum effects in liquid'
      endif
...
** Second-order:
      ...
      cv_e = CVtotE*9.32d8 ! C_V tot in erg/K/cm**3
      cv_ion = CVtotI*9.32d8 ! C_V tot in erg/K/cm**3


      return
      end

      function EOSPMAG(RHO,T6,B,Zion,CMI,Knonid)
*                                                       Version 13.09.12
* Stems from EOSPMAG v.25.05.08
* INPUT: density RHO [g/cc], temperature T6 [MK], m.f. B [rel.un.],
*        + effective charge and mass numbers (Zion and CMI)
*        Knonid - switch for Coulomb nonideality;
* OUTPUT: pressure EOSPMAG [Mbar]
      implicit double precision (A-H), double precision (O-Z)
      save
      parameter (AUM=1822.888d0,AUD=15819.7,UNIPR=1.42178d13,
     *  BOHR=137.036,PI=3.14159265d0,UNITEMP=5930.)
      parameter(GAMIMELT=175., ! OCP value of Gamma_i for melting
     *   RSIMELT=140.) ! ion density parameter of quantum melting
      data KDIR/0/ ! by default, the most favorable orientation of field
      DENRI=RHO/(AUD*AUM*CMI) ! number density of ions [rel.un.]
      TEMR=T6/UNITEMP ! Temperature in rel.un.
      TEMP=TEMR*BOHR**2 ! Temperature in a.u.
      RMI=CMI*AUM ! nucleus-to-electron mass ratio - 19.04.07
      DENR=DENRI*Zion  ! number density of free electrons
      PIDE=PRESSMAG(DENR,TEMR,B)
      if (Knonid.eq.-1) then ! without nonideality, but with degeneracy
         PIDI=RMI**4*PRESSMAG(DENRI/RMI**3,TEMR/RMI,B/RMI**2) ! spin=1/2
        goto 1
      endif
      GYRO=0.d0
      MULTI=1
      if (Z.eq.1.d0) then ! assume protons
         GYRO=5.5857d0
         MULTI=2
      endif
      call MAGNION(B,TEMR,DENRI,Zion,CMI,GYRO,MULTI,
     *   FIid,PIid,UIid,SIid,CVI,CHITI,CHIRI)
      PIDI=PIid*DENRI*TEMR
    1 continue
      PR=PIDE+PIDI
      if (Knonid.eq.-1) goto 2
      if (Knonid.lt.1.or.Knonid.gt.3) stop'EOSPMAG: Knonid?!'
      RS=(.75d0/(PI*DENRI*Zion))**.33333333d0/BOHR ! r_s
      Z13=Zion**.33333333d0
      RSI=RS*CMI*Zion**2*Z13*AUM ! R_S - ion density parameter
      GAME=1.d0/RS/TEMP ! electron Coulomb parameter Gamma_e
      GAMI=Zion**2*GAME/Z13 ! effective Gamma_i - ion Coulomb parameter
      ...
      KWK=0
      if (Knonid.ge.2) KWK=1
      GAMAG=B*BOHR**2 ! magnetic "gamma"
      if (Knonid.eq.3) GAMAG=0. ! test calc. without magnetic nonid.
      call EOSFIM12(LIQSOL,CMI,Zion,RS,GAMI,TEMP,GAMAG,KWK,KDIR,
     *  FC1,UC1,PC1,SC1,CV1,PDT1,PDR1,
     *  FC2,UC2,PC2,SC2,CV2,PDT2,PDR2)
      PRC=PC1*DENRI*TEMR ! Coul.pressure in rel.units
      PR=PR+PRC
    2 continue
      EOSPMAG=PR*UNIPR
      return
      ...
      function PRESSMAG(DENR,TEMR,B)
*                                                       Version 25.05.08
*   Input: DENR - electron density, 
*          TEMR - temperature, 
*          B - magnetic field
*   Output - electron gas pressure
* All quantities are in the relativistic units (m_e=c=\hbar=1)
      implicit double precision (A-H), double precision (O-Z)
      save
      call CHEMAG8(B,TEMR,DENR,
     *  DENR1,CHI,FEid,PEid,UEid,SEid,CVE,CHITE,CHIRE,DDENR)
      PRESSMAG=PEid*DENR*TEMR
      return
      end
