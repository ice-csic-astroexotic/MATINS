...
**   I M P R O V E M E N T S   O F   2 0 1 2 :
...
C%C      parameter(MAXY=10,UN_T6=.3157746)
...
C%C         write(*,'(''x ('',I2,''): ''$)') IX
C%C         read*,AY(IX)
C%C        if (AY(IX).le.0..or.ACMI(IX).le.0.) goto 2
C%C         NMIX=IX
C%C        if (AY(IX).eq.1.d0) goto 2
C%C      enddo
C%C    2 continue
...
C%C      enddo
C%C   10 continue
C%C      write(*,'('' Input lg(RHO) [g/cc], lg(T) (K) : ''$)')
C%C      read*,RHOlg,Tlg
...
*                                                       Version 21.01.10
*                       Cosmetic change (one warning retracted) 14.11.11
...
* the term x_i ln x_i = -S_{mix}/(N_i k).
     ...
     *     FION,UION,PION,CVii,PDTii,PDRii)
         ...
         PDTi=PDTii+1.d0
         PDRi=PDRii+1.d0
         FWK=TPT**2/24.d0 ! Wigner-Kirkwood (quantum diffr.) term
         UWK=2.d0*FWK
         CVWK=-UWK
         PWK=FWK
         PDRWK=2.d0*PWK
...
*   ----------------------------------------------------------------   *
     ...
     -       1.25d0*FJ1DXT-FJ2DX)/TEMP
      ...
      subroutine CHEMFIT0(DENS,TEMP,CHI)
*                                                       Version 07.06.07
