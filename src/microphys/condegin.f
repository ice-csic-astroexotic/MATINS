...
*   -------------------   RESTRICTIONS:   --------------------------   *
     ...
     & .or.CMI.le.0.) then
        print*,TEMP
        stop'CONDEGIN: Non-positive input parameter'
      endif
...
*   ----------------------   Preliminaries   -----------------------   *
      ...
      if (GAMI.lt.TINY**.2) then
        print*,'CONDIN: GAMI is too low: kappa>HUGE'
        stop
      endif
...
*   ---------------------- ion-ion:
      if (TRP.lt.1.d-4) then
         print*,'CONDIN: T is too low:kappa>HUGE'
         RKAPi=RKAPie
         stop
      else
...
c        print*,CL1,V**2,CL2,CL1-V**2*CL2
        if (I.eq.0) then ! 1st term calculated
           COULAN2=CL1-V**2*CL2
c        print*,'COULAN2=',COULAN2
